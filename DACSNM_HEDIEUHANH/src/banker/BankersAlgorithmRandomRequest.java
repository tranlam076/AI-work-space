...
public class BankersAlgorithmRandomRequest extends JFrame implements MouseListener {
	...
	public void paint(Graphics graphic) {
		...
		for (int i = startProcPannelY + disEachProcY; i <= startProcPannelY + disPannelY; i += disEachProcY) {
			...
			for (int i = 0; i < resNum; i++) {
				...
				if (changeAt == j && processArray[j].max[i] > processArray[j].need[i]) {
					...
					gg.fillRect(offset + startResAllocX + i * (disEachResX + space),
							...
							requestChange * ResMaxY_1 / UImaxY + 1);
	...
	private void go() throws InterruptedException {
		int curProc = 0;
		int[] curRequest = new int[resNum];
		Arrays.fill(curRequest, 0);
		...
		while (true) {
			...
			while (true && run) {
				...
				while (true && run) {
					...
					if (!safe) {
						message = "Process " + processTmp.id + " try to generate a request "
								+ Arrays.toString(request) + "... Unsafe state. request dinied!";
						for (int j = 0; j < request.length; j++) {
							processTmp.allocation[j] = processTmp.allocation[j] - request[j];
							processTmp.need[j] = processTmp.need[j] + request[j];
						}
