...
public class MayBay extends JFrame implements MouseListener {
	...
	public MayBay() throws IOException {
		...
		while (true) {
			...
			if (recieve.length() > 0) {
				if (recieve.contains("play-now")) {
					System.out.println("enemy attach: ");
					for (int i = 0; i < enemyAttach.size(); i++) {
						System.out.println(enemyAttach.get(i).x + enemyAttach.get(i).y);
					}
					play = true;
				}
				...
				if (recieve.contains("enemyAttach,")) {
					...
					System.out.println("" + Integer.parseInt(position[1].split("-")[0]) + "-"
							+ Integer.parseInt(position[1].split("-")[1]));
					this.repaint();
				}
				if (recieve.contains("Start,")) {
